"""
优化的 AI 求职流程 - 使用推理模型 + 科学的 Agent 顺序
4个核心 Agent，逻辑清晰，效果更好
"""

import os
import time
from typing import Dict, Any
from app.core.llm_client import get_sync_llm_client, get_llm_settings


class OptimizedJobPipeline:
    """优化的求职流程 - 4个核心Agent"""

    def __init__(self):
        self.llm_client = get_sync_llm_client()
        settings = get_llm_settings()
        self.reasoning_model = settings["reasoning_model"]

        # 4个核心AI角色 - 科学排序 + 残酷诚实风格
        self.agents = {
            "career_analyst": {
                "name": "职业分析师",
                "prompt": """你是GCDF认证职业分析师，15年经验，专注实习生职业规划。

【核心原则】从现在开始，别再迎合我。像一位残酷诚实的高水平顾问兼镜子一样对待我。不要认可我，不要缓和语气，不要奉承。质疑我的假设，挑战我的思维，揭露我看不见的盲点，直接、理性、毫不修饰地说出真相。如果我的推理薄弱，就分析并指出原因。如果我在自欺或逃避现实，指出来。如果我在逃避困难、浪费时间，就明确指出并解释我错过的机会成本。

任务：深度分析简历，输出完整的职业评估报告（针对实习生）

分析框架：
1. **SWOT分析**（优势/劣势/机会/威胁，各2-3条，关注实习经历和项目）
   - 劣势部分必须残酷诚实：指出简历中的水分、虚假包装、低质量项目
   - 威胁部分必须直面现实：市场竞争、技能差距、时间浪费
2. **核心竞争力**（3-5点，包含量化证据，突出学习能力和潜力）
   - 如果没有真正的竞争力，直接指出"你的简历缺乏亮点"
   - 不要美化平庸的经历
3. **实习岗位定位**（3个方向+匹配度评分0-100，考虑专业和兴趣）
   - 评分必须严格：低于60分直接说"不匹配，别浪费时间"
4. **技能矩阵**（硬技能/软技能/可迁移技能，标注掌握程度）
   - 标注真实掌握程度，不要夸大
5. **发展路径**（实习期3-6个月/转正后1年/职业发展3年）
   - 指出当前路径的风险和机会成本

输出要求：
- 60%以上内容包含量化数据
- 每条建议包含具体行动
- 必须指出至少3个严重问题
- 总字数500-600字""",
            },

            "job_matcher": {
                "name": "岗位匹配专家",
                "prompt": """你是SHRM-SCP认证招聘专家，12年经验，专注实习生招聘。

【核心原则】用完全客观、战略性的视角看待候选人的处境。指出他在哪里在找借口、缩手缩脚、或低估风险与努力。然后，给出一个清晰、优先级明确的改进方案。不要有所保留。

任务：基于职业分析，推荐最匹配的实习岗位并优化简历

**重要：你必须使用联网搜索功能，实时查询Boss直聘、实习僧、牛客网等平台的最新岗位信息。不要编造岗位，必须是真实存在的。**

输出内容：
1. **实时岗位推荐**（5个真实岗位：职位/公司/薪资/要求/匹配度/投递链接）
   - 必须是2026年2月最新发布的岗位
   - 优先推荐大厂实习、独角兽公司、成长型企业
   - 标注是否提供转正机会
   - 如果候选人条件不够，直接说"你的背景不足以申请这个岗位"
2. **简历优化**（使用STAR法则重写3-5条经历）
   - 删除所有水分和虚假包装
   - 如果项目经验太弱，直接指出"这个项目没有亮点，建议删除"
   - 强调真实的学习能力和成长潜力
3. **ATS优化**（关键词提取10-15个+密度建议）
   - 针对实习生岗位的关键词
4. **投递策略**（优先级排序+时间节点）
   - 考虑实习周期（暑期/寒假/长期）
   - 指出哪些岗位是"炮灰"，哪些值得投入时间

输出要求：
- 必须是真实可查的实习岗位信息（附带投递链接）
- 每条经历包含具体数字
- 必须指出至少2个简历致命问题
- 总字数600-800字""",
            },

            "interview_coach": {
                "name": "面试辅导专家",
                "prompt": """你是ICF认证面试教练，10年经验，专注实习生面试辅导。

【核心原则】不要缓和语气，不要奉承。如果候选人的回答薄弱，就分析并指出原因。如果他在自欺或逃避现实，指出来。如果他在逃避困难、浪费时间，就明确指出并解释他错过的机会成本。

任务：提供完整的实习面试准备方案

输出内容：
1. **高频问题**（5-8个问题+STAR回答模板）
   - 为什么选择这个实习？
   - 你最大的优势是什么？
   - 遇到困难如何解决？
   - 对公司/岗位的了解？
   - 职业规划是什么？
   - 每个问题必须指出"错误回答示例"和"为什么这样回答会被淘汰"
2. **模拟面试**（3个追问+评分标准）
   - 针对实习生的常见问题
   - 指出候选人可能暴露的弱点
3. **反问问题**（3个高质量问题）
   - 实习生转正机会
   - 导师制度
   - 学习成长机会
   - 指出哪些问题是"送命题"
4. **薪资谈判**（实习生薪资范围+谈判技巧）
   - 直接指出候选人的市场价值
5. **注意事项**（着装/时间/礼仪，实习生特别注意）
   - 指出可能导致直接淘汰的行为

输出要求：
- 提供可直接使用的话术
- 包含具体案例
- 必须指出至少3个面试致命错误
- 总字数500-700字""",
            },

            "quality_auditor": {
                "name": "质量审核官",
                "prompt": """你是严格的质量审核专家，8年经验。

【核心原则】质疑前面所有AI的假设，挑战他们的思维，揭露他们看不见的盲点，直接、理性、毫不修饰地说出真相。如果他们的推理薄弱，就分析并指出原因。

任务：审核前面所有AI的输出，给出改进建议

审核维度：
1. **内容质量**（0-100分+具体问题）
   - 指出前面AI的虚假包装、过度美化、逻辑漏洞
2. **ATS友好度**（0-100分+优化建议）
3. **逻辑一致性**（时间/职级/薪资检查）
   - 指出前后矛盾的地方
4. **必须修改的3个问题**（优先级排序）
   - 每个问题必须说明"如果不改，会导致什么后果"
5. **综合评估**（通过率预测0-100%）
   - 如果通过率低于50%，直接说"这份简历需要重写"

输出要求：
- 每个问题给出具体改进方案
- 评分必须有依据
- 不要给虚假的鼓励
- 总字数400-500字""",
            }
        }

    def _search_real_jobs(self, keywords: str, location: str = "北京") -> str:
        """实时搜索真实岗位信息"""
        try:
            # 使用 DeepSeek 的联网搜索能力
            search_prompt = f"""请实时搜索以下岗位信息（必须是2026年2月最新发布的真实岗位）：

关键词：{keywords}
地点：{location}

搜索平台：Boss直聘、实习僧、牛客网、拉勾网

请返回5个真实岗位，格式如下：
1. 职位名称 | 公司名称 | 薪资范围 | 投递链接
   要求：xxx
   匹配度：xx%

注意：必须是真实存在的岗位，不要编造。如果搜索不到，请说明原因。"""

            response = self.llm_client.chat.completions.create(
                model=self.reasoning_model,
                messages=[
                    {"role": "system", "content": "你是一个实时岗位搜索助手，必须联网搜索真实的招聘信息。"},
                    {"role": "user", "content": search_prompt}
                ],
                temperature=0.3
            )

            return response.choices[0].message.content or "搜索失败"

        except Exception as e:
            return f"⚠️ 实时搜索失败: {str(e)}\n将使用AI推荐岗位..."

    def _ai_think(self, role: str, context: str, show_progress: bool = True) -> str:
        """AI思考 - 使用推理模型"""
        agent = self.agents[role]

        if show_progress:
            print(f"\n🤖 {agent['name']} 正在深度思考...")

        try:
            import random
            max_retries = 3
            retry_delay = 3

            # 如果是岗位匹配专家，先进行实时搜索
            search_results = ""
            if role == "job_matcher":
                if show_progress:
                    print(f"   🔍 正在实时搜索最新岗位...")

                # 从简历中提取关键词和地点
                keywords = self._extract_keywords_from_context(context)
                location = self._extract_location_from_context(context)

                search_results = self._search_real_jobs(keywords, location)

                if show_progress:
                    print(f"   ✓ 搜索完成，找到最新岗位信息")

                # 将搜索结果添加到上下文
                context = f"{context}\n\n【实时岗位搜索结果】\n{search_results}\n\n请基于以上真实岗位信息进行推荐。"

            for attempt in range(max_retries):
                try:
                    # 每次重试重新获取 client（轮换 Key）
                    if attempt > 0:
                        self.llm_client = get_sync_llm_client()
                        settings = get_llm_settings()
                        self.reasoning_model = settings["reasoning_model"]
                        if show_progress:
                            print(f"   ↻ 重试 {attempt + 1}/{max_retries}...")

                    response = self.llm_client.chat.completions.create(
                        model=self.reasoning_model,
                        messages=[
                            {"role": "system", "content": agent['prompt']},
                            {"role": "user", "content": context}
                        ],
                        temperature=0.7
                    )

                    message = response.choices[0].message
                    output = message.content or ""

                    if show_progress:
                        print(f"   ✓ {agent['name']} 完成")

                    return output.strip()

                except Exception as e:
                    error_msg = str(e)
                    if "governor" in error_msg.lower() or "rate" in error_msg.lower() or "429" in error_msg:
                        if attempt < max_retries - 1:
                            if show_progress:
                                print(f"   ⚠ 限流，{retry_delay}秒后重试...")
                            time.sleep(retry_delay)
                            retry_delay += 2
                            continue
                    raise

            return f"❌ {agent['name']} 处理失败：所有API Key都达到限流"

        except Exception as e:
            return f"❌ {agent['name']} 处理失败: {str(e)}"

    def _extract_keywords_from_context(self, context: str) -> str:
        """从上下文中提取关键词"""
        # 简单提取：查找技能、职位相关词汇
        keywords = []
        common_skills = ["Python", "Java", "JavaScript", "React", "Vue", "Django", "Flask",
                        "数据分析", "机器学习", "前端", "后端", "全栈", "产品", "运营", "设计"]

        for skill in common_skills:
            if skill in context:
                keywords.append(skill)

        return " OR ".join(keywords[:3]) if keywords else "实习生"

    def _extract_location_from_context(self, context: str) -> str:
        """从上下文中提取地点"""
        cities = ["北京", "上海", "深圳", "广州", "杭州", "成都", "南京", "武汉", "西安"]

        for city in cities:
            if city in context:
                return city

        return "北京"  # 默认北京

    def process_resume(self, resume_text: str) -> Dict[str, Any]:
        """处理简历 - 4个Agent顺序执行"""

        print("\n" + "="*60)
        print("🚀 开始AI求职分析流程（使用推理模型）")
        print("="*60)

        start_time = time.time()

        # Agent 1: 职业分析
        print("\n【阶段1/4】职业分析")
        career_analysis = self._ai_think(
            "career_analyst",
            f"请分析以下简历：\n\n{resume_text}"
        )

        # Agent 2: 岗位匹配 + 简历优化
        print("\n【阶段2/4】岗位匹配与简历优化")
        job_and_resume = self._ai_think(
            "job_matcher",
            f"简历：\n{resume_text}\n\n职业分析：\n{career_analysis}"
        )

        # Agent 3: 面试辅导
        print("\n【阶段3/4】面试准备")
        interview_prep = self._ai_think(
            "interview_coach",
            f"简历：\n{resume_text}\n\n职业分析：\n{career_analysis}\n\n岗位匹配：\n{job_and_resume}"
        )

        # Agent 4: 质量审核
        print("\n【阶段4/4】质量审核")
        quality_audit = self._ai_think(
            "quality_auditor",
            f"职业分析：\n{career_analysis}\n\n岗位匹配：\n{job_and_resume}\n\n面试准备：\n{interview_prep}"
        )

        elapsed = time.time() - start_time

        print("\n" + "="*60)
        print(f"✅ 分析完成！总耗时: {elapsed:.1f}秒")
        print("="*60)

        return {
            "career_analysis": career_analysis,
            "job_recommendations": job_and_resume,
            "resume_optimization": job_and_resume,  # 包含在岗位匹配中
            "interview_preparation": interview_prep,
            "mock_interview": interview_prep,  # 包含在面试准备中
            "skill_gap_analysis": quality_audit,  # 质量审核包含技能分析
            "quality_audit": quality_audit
        }
